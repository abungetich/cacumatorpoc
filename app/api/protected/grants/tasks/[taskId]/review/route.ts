...
import { reviewGrantTask } from "@/lib/grants/service";
import { getRequestMetadata } from "@/lib/request-metadata";
import { buildValidationError, reviewGrantTaskSchema } from "@/lib/validation";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ taskId: string }> }) {
  ...
  const parsed = reviewGrantTaskSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(buildValidationError(parsed.error), { status: 400 });
  }

  const result = await reviewGrantTask({
    actor,
    taskId,
