...
import { submitRelationshipReview } from "@/lib/relationship-engine/service";
import { getRequestMetadata } from "@/lib/request-metadata";
import { buildValidationError, relationshipReviewSchema } from "@/lib/validation";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ mentorshipId: string }> }) {
  ...
  const parsed = relationshipReviewSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(buildValidationError(parsed.error), { status: 400 });
  }

  const result = await submitRelationshipReview({
    actor,
    mentorshipId,
    type: parsed.data.type as FeedbackType,
    rating: parsed.data.rating,
    strengths: parsed.data.strengths,
    areasForImprovement: parsed.data.areasForImprovement,
