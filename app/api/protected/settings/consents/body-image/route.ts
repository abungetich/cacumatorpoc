...
export async function POST(request: NextRequest) {
  ...
  if (!can(actor, "consents.manage")) {
    return NextResponse.json({ message: "Only platform admins can manage consent packs" }, { status: 403 });
  }
  ...
  const uploadDir = path.join(process.cwd(), "public", "uploads", "consents", "body-images");
  ...
  return NextResponse.json({
    ok: true,
    imageUrl: `/uploads/consents/body-images/${storedFileName}`,
  });
}
