...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ settingId: string }> }) {
  ...
  try {
    const item = await prisma.mentorConsentSetting.update({
      where: { id: settingId },
      data: {
    ...
    await prisma.auditLog.create({
      data: {
        userId: actor.id,
        action: "MENTOR_CONSENT_SETTING_UPDATED",
