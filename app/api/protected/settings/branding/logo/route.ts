...
export async function POST(request: NextRequest) {
  ...
  if (!can(actor, "platform.branding.manage")) {
    return NextResponse.json({ message: "Only platform admins can upload platform branding" }, { status: 403 });
  }
  ...
  if (!allowedMimeTypes.has(file.type)) {
    return NextResponse.json({ message: "Only PNG, JPG, WEBP, and SVG are allowed" }, { status: 400 });
  }
  ...
  return NextResponse.json({
    ok: true,
    logoUrl: `/uploads/platform/${storedName}`,
  });
}
