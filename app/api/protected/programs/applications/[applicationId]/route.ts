...
import { reviewMentorProgramApplication } from "@/lib/program-applications";
import { getRequestMetadata } from "@/lib/request-metadata";
import { buildValidationError, mentorProgramReviewSchema } from "@/lib/validation";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ applicationId: string }> }) {
  ...
  const parsed = mentorProgramReviewSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(buildValidationError(parsed.error), { status: 400 });
  }

  const { applicationId } = await params;
  const result = await reviewMentorProgramApplication(
    actor,
    applicationId,
    parsed.data.status,
